                     ...
                     int& offTDC )
{
    ...
    for ( int i = 0; i < numScintillator; i++ ) {
        ...
        for ( int j1 = ( i * muonTomoScope.scintEleNum );
              j1 < ( i * muonTomoScope.scintEleNum + muonTomoScope.scintEleNum ); j1++ ) {
                  ...
                  int TDCoff )
{
    ...
    int scintMax = -9999999, trigMax = scintMax, sigMax = scintMax;
    int scintMin = 99999999, trigMin = scintMin, sigMin = scintMin;
    ...
    for ( int ii = 0; ii < totData; ii++ ) {
        for ( int scintI = 0; scintI < scintNumN; scintI++ ) {
            for ( int j1 = ( scintI * scintSN ); j1 < ( scintI * scintSN + scintSN ); j1++ ) {
                ...
                if ( mt.ScintillatorArray[scintI].scintill[j].sensorVal->size () ) {
                    int mydata = mt.ScintillatorArray[scintI].scintill[j].sensorVal->at ( 0 );
    ...
    std::cout << " done upto this totDET = " << totDET << std::endl;
    for ( int ii = 0; ii < totData; ii++ ) {
...
int main ( int argc, char** argv )
{
    ...
    if ( configFileName = "" ) configFileName = "../../data/muonSim6133.tom";
