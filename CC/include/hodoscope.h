...
class Hodoscope
{
  public:
    ...
    void genLeaves (
        TTree* myTree, int numScintillator, int NumDetectors, int startModuleNum, int offTDC, int detChannelStart );
        ...
        while ( std::getline ( conFile, line ) ) {
            switch ( tot ) {
                case 22:
                    ...
                    if ( found != std::string::npos ) line.replace ( found, key.length (), "" );
